namespace EffectiveMobile
{
    ...
    {
        internal Dictionary<long, DeliveryRecord> _records;
        ...
        public RecordManager(string data)
            : this()
        {
            AddRecord(data);
        }
        ...
        public RecordManager(StreamReader reader)
            : this()
        {
            AddMultipleRecords(reader);
        }
            ...
            try
            {
                ...
                if (_records.ContainsKey(record.GetID()))
                {
                    throw new InvalidDataException(
                        $@"ID duplicate found: " + $@"[1]""{record.ToString(';')}""" + $@"[2]""{data}"""
                    );
                }
                else
                {
        ...
        public IEnumerable<DeliveryRecord> FilterRecords(string cityDistrict, DateTime firstDeliveryDateTime)
        {
            ...
            {
                Logger.Log(level: LogLevel.Notice, $@"There is no district ""{cityDistrict}"" in input file.");
